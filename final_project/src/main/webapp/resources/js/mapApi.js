//만날 장소 input
const spot=document.getElementById('schSpot');
...
if (navigator.geolocation) {
    ...
    navigator.geolocation.getCurrentPosition(function(position) {
		...
		// 마커에 커서가 오버됐을 때 마커 위에 표시할 커스텀 오버레이
		var content = '<div class="customoverlay" id="curPosition">현재 위치</div>';
		...
		kakao.maps.event.addListener(marker, 'mouseover', function() {
		  // 이벤트 발생하면 커스텀오버레이 보이게
		    customOverlay.setMap(map);
		});
		
		// 마커에 마우스아웃 이벤트를 등록합니다
		kakao.maps.event.addListener(marker, 'mouseout', function() {
		    // 이벤트 발생하면 커스텀오버레이 안보이게
...
function displayInfowindow(marker, title) {
   var content = '<div style="padding:5px;font-size:1.2em;">' + title + '</div>';
